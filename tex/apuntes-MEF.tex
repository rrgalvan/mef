...
%
\footnote{
  Como ejercicio, se puede comprobar que la función $g(x)=\exp(1/(x^2-1))$ si $|x|\le 1$, $g(x)=0$ si $|x|> 1$, es de $C^\infty$ y $\mathop{\rm sop}(g) = [-1,1]$, por tanto $g\in\TestSpace(\R)$. Pueden consultarse más ejemplos de funciones de $\TestSpace(\Omega)$,  por ejemplo, en~\cite{adams2003sobolev,sayas2019variational}.
}
%
...
\begin{enumerate}
  \item La derivada débil es única salvo conjuntos de medida nula. Por tanto, la derivada débil de $u\in L^2(\Omega)$ con respecto a la variable $x_i$ es la única función $u_{x_i}\in L^2(\Omega)$ que se caracteriza por:
\[
  \int_\Omega u_{x_i} \, \varphi\; = -\int_\Omega u\, \varphi_{x_i} \quad \forall \varphi \in \TestSpace(\Omega).
\]

  \item Si una función es derivable en sentido clásico, entonces existe su derivada débil y coincide (c.p.d.) con la clásica. En efecto, basta aplicar la fórmula de integración 
