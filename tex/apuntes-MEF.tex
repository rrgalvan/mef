...
\subsection*{Espacios de Lebesgue y Sobolev}
...
donde
\begin{equation*}
  \norm[L^p(\Omega)]{f} = \mathop{\rm ess\hspace{0.4ex}sup}(f) = \mathop{\rm inf} \{M\in\R+ \ /\ |f(x)| \le M \ \text{c.p.t}\ x\in\Omega \}.
\end{equation*}
...
\step 
Definimos el \deff{soporte} de una función $\phi:\Omega\to\R$ como el conjunto
\[
  \mathop{\rm sop}(\phi) = \overline{\{x\in\Omega \ /\ \phi(x)\neq 0\}},
\]
...
\[
  \int_\Omega v \phi(x)\; = -\int_\Omega u \partial_{x_i} \phi \quad \forall \phi \in C^\infty_0(\Omega).
\]
En este caso, decimos que $v$ es la \deff{derivada débil} de u\footnote{Para demostrar la unicidad de derivada débil véase, por ejemplo}~\cite{adams2003sobolev} y denotamos $v=\partial_{x_i}u$.

\step 
Es evidente que si $u\in C^1(\Omega)$ (es derivable en clásico), entonces existe su derivada débil y coincide con la clásica. 


\step
Definimos el \deff{espacio de Sobolev} $H^1(\Omega)$ como 
