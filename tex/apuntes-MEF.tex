...
\step{\textbf{Proposición}.} Para todo $f\in L^2(\Omega)$, existe una única solución del problema~\eqref{eq:problema.variacional.1d}, que verifica la siguiente acotación de estabilidad:
\[
  \norm[H_0^1]{u}\le \frac 1 {\alpha}\; \norm[L^2(\Omega)]{f}.
\]

\begin{proof}
  Idea:
  \begin{enumerate}
    \item $V=H_0^1(\Omega)$ es un espacio de Hilbert para el producto escalar $(\cdot,\cdot)_{H^1(\Omega)}$, pues son, respectivamente, una forma bilineal y una aplicación lineal 
    \item $a(\cdot,\cdot)$ y $L(\cdot)$ son respectivamente, una forma bilineal continua y una alicación lineal continua sobre $V$.
    \item La forma bilineal $a(\cdot,\cdot)$ es coercitiva.
    \item Podemos aplicar Lema de Lax-Milgram y concluir las tesis enunciadas en la proposición. 
  \end{enumerate}
\end{proof}

\step{\textbf{Proposición}.}
Si los datos son suficiente regulares ($f\in C^(\Omega)$), entonces la solución débil es una solución clásica del problema~\eqref{eq:problema.dirichlet.homogeneo.1d}.
